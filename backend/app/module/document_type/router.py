...
async def list_document_types(
    ...
    if current_user.is_superuser:
        doc_types = await doc_type_service.get_document_types(db)
    else:
        doc_types = await doc_type_service.get_document_types(db, organization_id=current_user.organization_id)
    return SuccessResponse(result=[DocumentTypeResponse.model_validate(d) for d in doc_types], message="Document types retrieved successfully", status_code=200)
