...
async def list_roles(
    ...
    if current_user.is_superuser:
        page = await role_service.get_roles(db, organization_id=organization_id, cursor=cursor, limit=limit)
    else:
        page = await role_service.get_roles(db, organization_id=current_user.organization_id, cursor=cursor, limit=limit)
    page.items = [RoleResponse.model_validate(r) for r in page.items]
    return SuccessResponse(result=page, message="Roles retrieved successfully", status_code=200)
